...
$testId = "T03"
$name = "Producto sin XML a cuarentena SinResolver"

try {
    Write-HHStep "$testId - $name"

    $pkgName = "HH_INVALID_NO_XML.zip"
    ...
    Wait-HHUntil -Description "producto sin XML en cuarentena SinResolver" -TimeoutSeconds $config.DefaultTimeout -Condition {
    ...
    Add-HHResult -Config $config -TestId $testId -Name $name -Result "PASS" -Detail "Producto sin XML en SinResolver."
