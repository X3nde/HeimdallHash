...
$testId = "T07"
$name = "Fichero extra no declarado en DN"

try {
    Write-HHStep "$testId - $name"

    $pkgName = "HH_INVALID_EXTRA_FILE.zip"
    ...
    Wait-HHUntil -Description "Producto con fichero extra no declarado enviado a cuarentena." -TimeoutSeconds $config.DefaultTimeout -Condition {
    ...
    Add-HHResult -Config $config -TestId $testId -Name $name -Result "PASS" -Detail "Producto con fichero extra no declarado enviado a cuarentena."
